...
def render_main(w, wifi):
    ...
    # Header
    draw.text((12, 8), CITY.upper(), font=f(13), fill=(80, 80, 95))
    draw.text((12, 24), time.strftime("%a %d %b"), font=f(11), fill=(55, 55, 70))
    draw_wifi(draw, 216, 10, wifi)
    draw.line([(12, 42), (228, 42)], fill=(25, 25, 35), width=1)
    
    # Big temperature
    ts = f"{w['temp']}°"
    tw = draw.textlength(ts, font=f(85))
    draw.text(((240 - tw) / 2, 48), ts, font=f(85), fill=tc)
    
    # Condition
    cond = WMO.get(w['code'], 'Unknown')
    cw = draw.textlength(cond, font=f(16))
    draw.text(((240 - cw) / 2, 138), cond, font=f(16), fill=(200, 200, 210))
    
    # Feels like
    fl = f"Feels {w['feels']}°"
    fw = draw.textlength(fl, font=f(12))
    draw.text(((240 - fw) / 2, 160), fl, font=f(12), fill=(70, 70, 85))
    ...
    draw.text((hx - draw.textlength(hl_text, font=f(18))/2, 208), hl_text, font=f(18), fill=(255, 160, 80))
    
    lx = center + spacing
    draw.polygon([(lx-8, 194), (lx, 204), (lx+8, 194)], fill=(100, 160, 255))
    draw.text((lx - draw.textlength(ll_text, font=f(18))/2, 208), ll_text, font=f(18), fill=(120, 180, 255))
    
    # Clock
    clk = time.strftime("%H:%M")
    draw.text(((240 - draw.textlength(clk, font=f(10))) / 2, 228), clk, font=f(10), fill=(40, 40, 55))
...
def render_left(w, wifi):
    ...
    # Humidity
    draw.text((8, 4), "HUM", font=f(10), fill=(50, 50, 65))
    draw.text((8, 16), f"{w['humidity']}%", font=f(24), fill=(60, 180, 180))
    draw_bar(draw, 8, 52, 60, 5, w['humidity']/100, (60, 180, 180))
    
    draw.line([(80, 8), (80, 72)], fill=(25, 25, 35), width=1)
    
    # Wind
    draw.text((88, 4), "WIND", font=f(10), fill=(50, 50, 65))
    draw.text((88, 16), f"{w['wind']}", font=f(24), fill=(160, 110, 220))
    draw.text((88, 46), "km/h", font=f(9), fill=(60, 60, 75))
    draw.text((88, 60), wind_dir(w['wdir']), font=f(12), fill=(160, 110, 220))
...
def render_right(w, wifi):
    ...
    draw.text((32 - draw.textlength(w['sunrise'], font=f(14))/2, 50), 
              w['sunrise'], font=f(14), fill=(255, 190, 80))
    
    draw.line([(80, 8), (80, 72)], fill=(25, 25, 35), width=1)
    
    # Sunset
    draw_sunset(draw, 120, 28, r=14)
    draw.text((120 - draw.textlength(w['sunset'], font=f(14))/2, 50), 
              w['sunset'], font=f(14), fill=(255, 110, 60))
